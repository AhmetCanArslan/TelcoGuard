...
| Rol | Kim? | Ne Yapar? |
|-----|------|-----------|
| **NOC Operatörü** | 7/24 şebeke izleyen teknisyen | Dashboard izler, alarmları değerlendirir, saha mühendisine atar, location based olacak, konuma göre yakın olan saha mühendisine atama yapabilecek, sadece bu atama yapabilecek.  |
| **Saha Mühendisi** | Sahada müdahale eden mühendis | Atanan alarmları görür, müdahale eder, çözüm notu yazar, prop location koyacağız NOC ona göre atama yapacak, operasyonel uygunluk müsaitlik, aktif mi çevrim içi mi (websocket varsa çevrim içi, ne zaman verilen işi kabul etti, ne zaman reddetti, detaylı loglama, en son çevrimiçi olma, bilidirm, iş tamamlama süresi, çözümde kullanılan sarf giderler madde olarak giderleri sıralayacak oradan seçecek, arızayı foto çekme ve yükleme, sesli arama öncelik düşük,  |
| **Şebeke Yöneticisi** | Stratejik karar verici | Özet dashboard görür, trend ve bölge analizi yapar, saha mühendisi iş analistliği rapur aylık kaç saat çalıştı kaç arıza çözdü ortalama arıza çözme süresi skor |
| **RAPORLAMA** | Hasar raporları | detaylı olarak hangi bölge en çok sıkıntı çıkyoruz, hangi bölgede hangi sorunlar daha fazla oluyor, en çok sroun olan bölge, en çok sorun yapan maddesel (kablo, wifi adapter fln)şehir/bölge düzeyinde sağlık skoru - SLA takibi: uptime yüzdesi hesaplama ve gösterimi, - Export: alarm ve metrik verilerini CSV olarak indirme
...
**Desteklenmesi gereken anomali senaryoları :**
...
Simülatörün ölçüm göndermesi üretmesi yetmez — sisteminizin metrikleri analiz edip anormal durumları tespit etmesi gerekiyor. Aşağıdaki yöntemlerden en az birini uygulayın: Alarm önceliklendirme: etkilenen kullanıcı sayısına göre otomatik öncelik
...
> **Not:**  Eşik değerleri veritabanında konfigüre edilebilir olmalı, hard-coded olmamalı.
...
- **Tekrar eden alarm birleştirme:** aynı istasyonun aynı metriki için 5dk içinde yeni alarm açılmamalı, mevcut alarm güncellensin alarm olup olmadığı kontrol edilecek

### 2.6 Dashboard ve Görselleştirme

- **Canlı özet panel:** toplam istasyon, aktif alarm sayısı, kritik alarm sayısı, çevrimiçi saha mühendisi sayısı, harita görünümü  istasyonlar durumuna göre yeşil/sarı/kırmızı/gri pin compact görünüm olacak, çözülen çözülmekte olan ve çözülecek olan alarm sayıları leaflet openstreetmap backedn react-leaflet

- **İstasyon detay paneli:** tıklanan istasyonun tüm metriklerinin zaman serisi grafikleri (son 5-10 dakika hisse gibi 7 saat 10 gün fln filan)
...
- Telemetri verisi için ek olarak zaman serisi DB NoSQL (Redis) 
...
### 5.4 Simulator Control ayrı folderda microservice mimariyle yapacağız
...
| Rol | Kim? | Ne Yapar? |
|-----|------|-----------|
| **NOC Operatörü** | 7/24 şebeke izleyen teknisyen | Dashboard izler, alarmları değerlendirir, saha mühendisine atar, location based olacak, konuma göre yakın olan saha mühendisine atama yapabilecek, alarm derecesi 1-5,  |
| **Saha Mühendisi** | Sahada müdahale eden mühendis | Atanan alarmları görür, müdahale eder, çözüm notu yazar, prop location koyacağız NOC ona göre atama yapacak, operasyonel uygunluk müsaitlik, aktif mi çevrim içi mi (websocket varsa çevrim içi, ne zaman verilen işi kabul etti, ne zaman reddetti, detaylı loglama, en son çevrimiçi olma, bilidirm, iş tamamlama süresi, çözümde kullanılan sarf giderler, arızayı foto çekme ve yükleme, sesli arama öncelik düşük,  |
| **Şebeke Yöneticisi** | Stratejik karar verici | Özet dashboard görür, trend ve bölge analizi yapar, saha mühendisi iş analistliği rapur aylık kaç saat çalıştı kaç arıza çözdü ortalama arıza çözme süresi skor
